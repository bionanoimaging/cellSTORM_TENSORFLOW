...
void onRunDone(void *userdata, int taskStamp) {
    ...
    const char *output1 = strdup("hello world");
    const char *output2 = strdup("hello sun");

    jobjectArray result = (jobjectArray) env->NewObjectArray(2, env->FindClass("java/lang/String"),
                                                             env->NewStringUTF(""));
    env->SetObjectArrayElement(result, 0, env->NewStringUTF(output1));
    env->SetObjectArrayElement(result, 1, env->NewStringUTF(output2));
...
Java_de_nanoimaging_deconv_ModelManager_registerListenerJNI(JNIEnv *env, jclass type,
                                                          jobject callbacks) {
...
Java_de_nanoimaging_deconv_ModelManager_loadModelAsync(JNIEnv *env, jclass type,
...
extern "C"
JNIEXPORT void JNICALL
Java_de_nanoimaging_deconv_ModelManager_runModelAsync(JNIEnv *env, jclass type,
...
extern "C"
JNIEXPORT void JNICALL
Java_de_nanoimaging_deconv_ModelManager_unloadModelAsync(JNIEnv *env, jclass type) {
    if (NULL == modelManager) {
        LOGE("please load model first");
